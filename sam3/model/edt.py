...
if _TRITON_AVAILABLE:
    ...
    def edt_triton(data: torch.Tensor):
        """
        CPU fallback for Euclidean Distance Transform using scipy.
        Used on platforms where triton is unavailable (e.g. macOS).
        ...
        assert data.dim() == 3
        B, H, W = data.shape
        data_np = data.bool().cpu().numpy()
        result = torch.zeros(B, H, W, dtype=torch.float32)
        for i in range(B):
            result[i] = torch.from_numpy(
                scipy.ndimage.distance_transform_edt(data_np[i]).astype("float32")
            )
